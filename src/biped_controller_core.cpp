...
BipedControllerCore::getActuationData(int idx) {
  ...
  if (config_.fwddyn) {
    ...
    crocoddyl::DataCollectorJointActMultibodyInContact *multibody =
        &ddata->multibody;
    actuation_data = multibody->actuation;
  } else {
    std::shared_ptr<crocoddyl::DifferentialActionDataContactInvDynamics> ddata =
        std::dynamic_pointer_cast<
            crocoddyl::DifferentialActionDataContactInvDynamics>(
            action_data->differential);
    crocoddyl::DataCollectorJointActMultibodyInContact *multibody =
        &ddata->multibody;
    actuation_data = multibody->actuation;
...
BipedControllerCore::createWalkingProblem(const Eigen::VectorXd &x0,
...
BipedControllerCore::createSwingFootModel(
  ...
  if (comTask != Eigen::Vector3d::Zero()) {
    ...
    costModel->addCost("comTrack", comTrack,
                       config_.cost_weight.com_track_weight);
  }
  ...
  for (size_t i = 0; i < supportFootIds.size(); ++i) {
    ...
    costModel->addCost(model_->frames[supportFootIds[i]].name + "_wrenchCone",
                       wrench_cone, config_.cost_weight.contact_wrench_weight);
  }
  ...
  if (!swingFootTask.empty()) {
    for (size_t i = 0; i < swingFootTask.size(); i++) {
      ...
      costModel->addCost(model_->frames[id].name + "_footTrack", foot_track,
                         config_.cost_weight.foot_track_weight);
    }
  }

  // centroidal momentum regularization cost
  std::shared_ptr<crocoddyl::ResidualModelAbstract>
      centroidal_momentum_residual =
          std::make_shared<crocoddyl::ResidualModelCentroidalMomentum>(
              state_, Eigen::VectorXd::Zero(6), nu);
  ...
  costModel->addCost("centroidalMomentumReg", centroidalMomentumReg,
                     config_.cost_weight.centroidal_momentum_weight);
