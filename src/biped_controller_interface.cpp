...
BipedControllerInterface::BipedControllerInterface(ros::NodeHandle& nh, ros::NodeHandle& pnh)
  : nh_(nh), pnh_(pnh), core_(std::make_shared<BipedControllerCore>(loadCoreConfig(nh, pnh)))
{
  ...
  contact_wrench_pub_ = nh_.advertise<geometry_msgs::WrenchStamped>("contact_wrenches", 10);
  ...
  solver_statistics_pub_ = nh_.advertise<std_msgs::Float64MultiArray>("solver_statistics", 10);
  ...
  for (const auto& foot : { core_->getConfig().lleg, core_->getConfig().rleg })
  {
    ...
    static_transform.child_frame_id = tf::resolve(tf_prefix_, foot + "_contact");
    static_tf_broadcaster_.sendTransform(static_transform);
  }
}

BipedControllerCore::Config BipedControllerInterface::loadCoreConfig(ros::NodeHandle& nh, ros::NodeHandle& pnh)
{
  BipedControllerCore::Config config;
  while (!nh.getParam("robot_description", config.robot_description))
    ROS_ERROR_STREAM_THROTTLE(1.0, "Failed to get robot_description from ros parameter server");
  ...
  std::cout << "lleg: " << config.lleg << ", rleg: " << config.rleg << ", root link: " << config.root_link_name
            << std::endl;

  int solver_type = (int)config.solver_type;
  pnh.param<int>("solver_type", solver_type, solver_type);
  config.solver_type = static_cast<BipedControllerCore::SolverType>(solver_type);
...
void BipedControllerInterface::publishContactWrenches(int pub_idx)
{
  ...
  std::shared_ptr<crocoddyl::ContactModelMultiple> contacts_model = contacts.first;
  std::shared_ptr<crocoddyl::ContactDataMultiple> contacts_data = contacts.second;
  const auto& config = core_->getConfig();
  const auto& us = core_->getUs();
  const auto& model = core_->getModel();

  std::map<std::string, std::shared_ptr<crocoddyl::ContactItem>> contact_container = contacts_model->get_contacts();
  std::map<std::string, std::shared_ptr<crocoddyl::ContactDataAbstract>> contact_data_container =
  ...
  {
    std::shared_ptr<crocoddyl::ContactModelAbstract> contact_model = contact_container[active_contact]->contact;
    std::shared_ptr<crocoddyl::ContactDataAbstract> contact_data = contact_data_container[active_contact];

    Eigen::VectorXd f_contact = Eigen::VectorXd::Zero(6);

    if (auto m6d = std::dynamic_pointer_cast<crocoddyl::ContactModel6D>(contact_model))
    {
      auto d6d = std::dynamic_pointer_cast<crocoddyl::ContactData6D>(contact_data);
      if (config.fwddyn)
      {
        switch (m6d->get_type())
        {
    ...
    else if (auto m3d = std::dynamic_pointer_cast<crocoddyl::ContactModel3D>(contact_model))
    {
      auto d3d = std::dynamic_pointer_cast<crocoddyl::ContactData3D>(contact_data);
      if (config.fwddyn)
      {
    ...
    contact_wrench_msg.header.frame_id = tf::resolve(tf_prefix_, active_contact);
...
void BipedControllerInterface::publishRootPose(int pub_idx)
{
  ...
  robot_base_transform.child_frame_id = tf::resolve(tf_prefix_, core_->getConfig().root_link_name);
...
{
  std::shared_ptr<crocoddyl::ActuationDataAbstract> actuation_data = core_->getActuationData(pub_idx);
  const auto& config = core_->getConfig();
  const auto& model = core_->getModel();
  const auto& xs = core_->getXs();
  const auto& us = core_->getUs();
  ...
  for (pinocchio::JointIndex i = 2; i < model->njoints; i++)
  {
    ...
    joint_state.effort.push_back(config.fwddyn ? us.at(pub_idx)(v_index - 6) : actuation_data->u(v_index - 6));
  }
...
void BipedControllerInterface::publishFootPoses(int pub_idx)
{
  ...
  lfoot_pose_msg.pose.position.x = data->oMf[model->getFrameId(config.lleg)].translation().x();
  lfoot_pose_msg.pose.position.y = data->oMf[model->getFrameId(config.lleg)].translation().y();
  lfoot_pose_msg.pose.position.z = data->oMf[model->getFrameId(config.lleg)].translation().z();
  Eigen::Quaterniond lfoot_quat(data->oMf[model->getFrameId(config.lleg)].rotation());
  ...
  rfoot_pose_msg.pose.position.x = data->oMf[model->getFrameId(config.rleg)].translation().x();
  rfoot_pose_msg.pose.position.y = data->oMf[model->getFrameId(config.rleg)].translation().y();
  rfoot_pose_msg.pose.position.z = data->oMf[model->getFrameId(config.rleg)].translation().z();
  Eigen::Quaterniond rfoot_quat(data->oMf[model->getFrameId(config.rleg)].rotation());
...
void BipedControllerInterface::publishSolverStatistics()
{
  ...
  solver_stats_msg.data.push_back(core_->getLastSolveTime() / solver->get_iter());
